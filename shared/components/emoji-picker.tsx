...
import { Activity, Clock, Flag, Lightbulb, MapPin, Search, Smile, Users } from 'lucide-react';
...
const EmojiGrid = ({
	...
	<div className='grid grid-cols-8 gap-1 p-2' ref={emojiGridRef}>
		{emojiList.map((emoji, index) => {
			const globalIndex = showCategory ? allVisibleEmojis.findIndex((e) => e.emoji === emoji.emoji) : index;
...
export default function EmojiPicker({ onEmojiSelect, trigger, maxRecentEmojis = 24 }: EmojiPickerProps) {
	...
	const filteredEmojis = useMemo(() => {
		...
		return emojis.filter((emoji) => {
			...
			const emojiKeywords = 'keywords' in emoji && Array.isArray(emoji.keywords) ? emoji.keywords : [];
			const keywordMatch = emojiKeywords.some((keyword: string) => keyword.toLowerCase().includes(searchLower));
		...
		<Popover open={isOpen} onOpenChange={setIsOpen}>
			...
			<PopoverContent className='w-80 p-0' align='end' onKeyDown={handleKeyDown}>
				...
				{searchTerm ? (
					...
					/* Category Tabs */
					<Tabs defaultValue={recentEmojis.length > 0 ? 'recent' : categories[0]} className='w-full'>
						<TabsList
						...
						>
							...
							{categories.slice(0, recentEmojis.length > 0 ? 3 : 4).map((category) => {
								const IconComponent = categoryIcons[category as keyof typeof categoryIcons] || Smile;
								return (
									<TabsTrigger
										key={category}
										value={category}
