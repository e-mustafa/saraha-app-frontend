...
export default function SettingsPage() {
	...
	const handleBrandThemeChange = (newTheme: 'violet' | 'emerald' | 'sunset' | 'ocean') => {
		...
		toast.success(t('theme.themePaletteChangedTo', { theme: t(`theme.palettes.${newTheme}`) }));
	};

	const handleDisplayModeChange = (mode: 'light' | 'dark' | 'system') => {
		setDisplayMode(mode);
		toast.success(t('theme.displayModeSetTo', { mode: t(`theme.modes.${mode}`) }));
	};
	...
	const handleLanguageChange = (newLanguage: 'ar' | 'en') => {
		...
		toast.success(t('theme.languageSetTo', { language: newLanguage === 'ar' ? 'العربية' : 'English' }));
	};
	...
	return (
		<div className='min-h-full relative overflow-hidden'>
			...
			<div className='relative mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8'>
				<div className='mb-8'>
					<h1 className='text-4xl font-bold tracking-tight text-foreground mb-2'>{t('settingsTitle')}</h1>
				...
				<div className='space-y-8'>
					...
					<div className='backdrop-blur-xl bg-card-glass/80 rounded-2xl shadow-2xl border border-brand-primary/20 p-8'>
						...
						<div className='grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4'>
							{[
								{ id: 'violet' as const, name: t('theme.palettes.violet'), color: 'from-violet-500 to-purple-600' },
								{
									id: 'emerald' as const,
									name: t('theme.palettes.emerald'),
									color: 'from-emerald-500 to-green-600',
								},
								{ id: 'sunset' as const, name: t('theme.palettes.sunset'), color: 'from-orange-500 to-red-600' },
								{ id: 'ocean' as const, name: t('theme.palettes.ocean'), color: 'from-cyan-500 to-blue-600' },
							].map((themeOption) => {
